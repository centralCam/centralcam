                    ...
                    <Slider {...settings} className='flex items-center justify-center'>
                        {dataCarrusel.map((item, i) => (
                            <div key={i} className='flex items-center justify-center m-2 md:bg-lightpink gap-4'>
                                <img src={item.imgSrc} alt={`Carousel image ${i + 1}`} width={96} height={96} className="w-auto md:h-32" loading="lazy" title={`Carousel image ${i + 1}`} aria-label={`Carousel image ${i + 1}`} />
                            </div>
                        ))}
                    </Slider>
                </div>
